...
def write_edge_route_table(html_routes):
    """The two runtimes that negotiate Accept need to know which HTML routes have
    a markdown twin. Generated so neither table can drift from what this script
    emitted.

    The .ts is imported by netlify/edge-functions/markdown.ts and runs on Deno,
    which is why it is committed. The .json is read by server/index.mjs, the
    Node server the Render deploy runs, and lives in build/ with the rest of the
    intermediates because that deploy builds from source before it serves."""
    d = ROOT / "netlify" / "edge-functions" / "lib"
    d.mkdir(parents=True, exist_ok=True)
    table = json.dumps(html_routes, indent=2, sort_keys=True)
    body = (
        "// Generated by scripts/build_md.py. Do not edit.\n"
        "// HTML route -> markdown twin, for the Accept negotiation edge function.\n"
        "export const MD_TWINS: Record<string, string> = " + table + ";\n"
    )
    (d / "md-routes.ts").write_text(body, encoding="utf-8")
    (BUILD / "md-routes.json").write_text(table + "\n", encoding="utf-8")
