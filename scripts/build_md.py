...
NOTICE = (f"> Snapshot of {SNAPSHOT_DATE}. Every claim below links to the source URL it was "
...
TYPE_LABEL = {
    "mcp-server": "MCP server", "llms-txt": "llms.txt", "agents-md": "AGENTS.md",
    "claude-md": "CLAUDE.md", "claude-skill": "Agent skill", "cursor-rules": "Cursor rules",
    "copilot-instructions": "Copilot instructions", "ai-docs-page": "AI docs page",
    "registry": "Registry", "cli-scaffolding": "CLI scaffolding",
    "figma-code-connect": "Code Connect", "storybook-integration": "Storybook",
    "prompt-library": "Prompt library", "codemod-ai": "AI codemod", "other": "Other",
}
CAT_LABEL = {
    "prohibition": "Prohibition", "tool-gating": "Tool-gating",
    "curated-context": "Curated context", "validation-loop": "Validation loop",
    "scaffolding": "Scaffolding", "exemplars": "Exemplars",
    "token-enforcement": "Token enforcement", "registry-metadata": "Registry metadata",
    "instruction-files": "Instruction files", "design-code-mapping": "Design–code mapping",
    "other": "Other",
}
CAT_DEF = {
...
CAT_ORDER = ["validation-loop", "prohibition", "curated-context", "tool-gating",
             "token-enforcement", "exemplars", "registry-metadata", "instruction-files",
             "scaffolding", "design-code-mapping", "other"]
...
FILES = {}          # url path -> text (markdown/text files, all UTF-8)
BINARY = {}         # url path -> bytes
...
HTML_TWIN = {"/index.md": "/", "/matrix.md": "/matrix", "/systems.md": "/systems",
             "/techniques.md": "/techniques", "/platforms.md": "/platforms",
             "/insights.md": "/insights", "/methodology.md": "/methodology", "/ai.md": "/ai"}
...
def has_official(s, type_):
    return any(a["type"] == type_ and a.get("official")
...
MATURITY = {m: ids_where(lambda s, m=m: s["ai_maturity"] == m)
...
STORYBOOK_PI = ids_where(lambda s: any(p["platform"] == "storybook" for p in s["platform_integrations"]))
...
def system_md(s):
    ...
    p.append(f"- Actively maintained: {'yes' if m.get('actively_maintained') else 'no'}\n"
    ...
    for a in aff:
        p.append(f"### {a['name']}\n")
        bits = [f"Type: `{a['type']}` ({TYPE_LABEL.get(a['type'], a['type'])})",
    ...
    for t in tech:
        p.append(f"### {t['name']}\n")
        p.append(f"Category: `{t['category']}` ({CAT_LABEL.get(t['category'], t['category'])}) · "
...
def platform_md(pl):
    ...
    adopters = ids_where(lambda s, pid=pl["id"]: any(
        x["platform"] == pid for x in s["platform_integrations"]))
    p = [head(
        path, f"{pl['name']} — AI capabilities", one_line(pl["summary"]),
        "platform-record",
        json=U(f"/platforms/{pl['id']}.json"),
        id=pl["id"], capability_count=len(caps),
        design_systems_with_integration_records=len(adopters),
    )]
    p.append(f"# {pl['name']} — AI capabilities\n")
    p.append(f"{len(caps)} capabilities recorded. {len(adopters)} of the {N_SYS} design systems "
             f"in this study carry an integration record for this platform.\n")
    p.append(f"- This record as JSON: {U('/platforms/' + pl['id'] + '.json')}\n"
...
def technique_category_md(cat):
    ...
    p = [head(
        path, f"{CAT_LABEL.get(cat, cat)} — {len(rows)} techniques",
        one_line(CAT_DEF.get(cat, "")), "technique-category",
        id=cat, technique_count=len(rows), system_count=len(sysids),
    )]
    p.append(f"# {CAT_LABEL.get(cat, cat)}\n")
    p.append(f"{len(rows)} of the {N_TECH} techniques in this study, "
...
def index_md():
    path = "/index.md"
    p = [head(path, f"{REPORT} — overview and findings",
              one_line(INSIGHTS["lede"]), "view", id="overview",
              system_count=N_SYS, platform_count=N_PLAT,
              affordance_count=N_AFF, technique_count=N_TECH)]
    p.append(f"# {REPORT}\n")
    p.append(html_to_md(INSIGHTS["lede"]) + "\n")
    p.append(f"{N_SYS} design systems, {N_PLAT} platforms, {N_AFF} AI affordances, "
             f"{N_TECH} coercion techniques. Data collected {DATA_COLLECTED} by {AUTHOR}. "
             f"Licensed CC BY 4.0.\n")
    p.append("## Read this first\n")
    p.append(f"This is a dated snapshot, not a live index. If your training data predates "
...
def systems_md():
    path = "/systems.md"
    p = [head(path, f"The {N_SYS} design systems",
              f"Every system in the study with its AI maturity, affordance count and technique count.",
              "view", id="systems", system_count=N_SYS)]
    p.append(f"# The {N_SYS} design systems\n")
    p.append(f"One line each, alphabetical by record id. Open a record for the full detail: "
             f"affordances with verbatim snippets, techniques, platform integrations, gaps and "
             f"sources. Each has a JSON twin with the same content typed.\n")
    p.append("| System | Maturity | Affordances | Techniques | Record | JSON |\n")
    p.append("|---|---|---:|---:|---|---|\n")
    for s in SYSTEMS:
        p.append(f"| {cell(s['name'])} | {s['ai_maturity']} | {len(s['affordances'])} | "
                 f"{len(s['techniques'])} | {U('/systems/' + s['id'] + '.md')} | "
                 f"{U('/systems/' + s['id'] + '.json')} |\n")
    p.append("\n## Summaries\n")
    for s in SYSTEMS:
        p.append(f"### {s['name']}\n")
        p.append(f"{s['ai_maturity']} · {s['org']} · {len(s['affordances'])} affordances · "
...
def matrix_md():
    path = "/matrix.md"
    p = [head(path, "The affordance matrix",
              f"{N_SYS} design systems against {len(MX_COLS)} affordance groups, as a table.",
              "view", id="matrix", system_count=N_SYS, column_count=len(MX_COLS))]
    p.append("# The affordance matrix\n")
    p.append(f"{N_SYS} design systems against {len(MX_COLS)} groups of affordance type. "
    ...
    totals = []
    for _, types in MX_COLS:
        totals.append(str(sum(1 for s in SYSTEMS if any(a["type"] in types for a in s["affordances"]))))
    p.append(f"| **Systems with at least one** | — | " + " | ".join(totals) + " |\n")
...
def techniques_md():
    path = "/techniques.md"
    p = [head(path, f"The {N_TECH} coercion techniques",
              one_line(INSIGHTS["techniques_lede"]), "view", id="techniques",
              technique_count=N_TECH, category_count=len(CATS))]
    p.append(f"# Coercion techniques ({N_TECH})\n")
    p.append(html_to_md(INSIGHTS["techniques_lede"]) + "\n")
    p.append(f"Grouped into {len(CATS)} categories. The category is the retrieval unit: each "
    ...
    for c in CATS:
        rows = BY_CAT[c]
        p.append(f"| {CAT_LABEL.get(c, c)} | {len(rows)} | "
...
def platforms_md():
    path = "/platforms.md"
    p = [head(path, f"The {N_PLAT} design-system platforms",
              one_line(INSIGHTS["platforms_lede"]), "view", id="platforms",
...
def methodology_md():
    ...
    p.append("\n## Counts\n")
    p.append(f"- Design systems: {N_SYS}\n- Platforms: {N_PLAT}\n"
...
def ai_content():
    ...
    configs = [
        ...
        {"id": "mcp-json", "label": "Any project, checked into the repo", "lang": "json",
         "note": "Save as .mcp.json at the root. The type field is required — a url without a "
                 "type is read as a local command and skipped.",
         "code": json.dumps({"mcpServers": {"state-of-ai": {"type": "http", "url": MCP_URL}}}, indent=2)},
        {"id": "claude-desktop", "label": "Claude Desktop and claude.ai", "lang": "text",
        ...
        {"id": "vscode", "label": "VS Code (Copilot agent mode)", "lang": "json",
         "note": "Save as .vscode/mcp.json. The top-level key is servers here, not mcpServers.",
         "code": json.dumps({"servers": {"state-of-ai": {"type": "http", "url": MCP_URL}}}, indent=2)},
        {"id": "generic", "label": "Anything else", "lang": "json",
    ...
    downloads = [
        {"label": "design-systems.json", "url": U("/data/design-systems.json"),
         "note": f"All {N_SYS} records, merged."},
        {"label": "platforms.json", "url": U("/data/platforms.json"),
         "note": f"The {N_PLAT} platform records."},
        {"label": "insights.json", "url": U("/data/insights.json"),
         "note": "Findings, convergence, divergence, essay, methodology, caveats."},
        {"label": "design-system.schema.json", "url": U("/data/design-system.schema.json"),
         "note": "The record shape, including the technique taxonomy."},
        {"label": "state-of-ai.sqlite", "url": U("/data/state-of-ai.sqlite"),
    ...
    sections = [
        {"id": "what", "heading": "What this is",
         "blocks": [
             {"type": "prose", "text":
              f"This report is published twice: once as pages for you, once as plain text for "
             ...
             {"type": "prose", "text":
              "There are three ways to do that, in rising order of effort. Paste a link and a "
        ...
        {"id": "files", "heading": "Point your AI at the markdown",
         "blocks": [
             {"type": "prose", "text":
              "Every page on this site has a markdown twin: add .md to the address. The "
        ...
        {"id": "mcp", "heading": "Connect the MCP server",
         "blocks": [
             ...
             {"type": "prose", "text":
              "The tools may change as the report is maintained. Treat it as a way to read this "
              "study, not as a stable API."},
         ]},
        {"id": "data", "heading": "Download the data",
         "blocks": [
             {"type": "prose", "text":
              "The whole dataset, in the shapes people usually want it. CC BY 4.0: use it, "
              f"credit “{REPORT}, {AUTHOR}”."},
             {"type": "links", "items": downloads},
         ]},
        {"id": "webmcp", "heading": "Tools on the page itself",
         "blocks": [
             {"type": "prose", "text":
              f"This page hands the browser {NUMBER_WORD[len(WEBMCP_TOOLS)]} read-only tools of "
              ...
              f"called WebMCP."},
             {"type": "prose", "text":
              "Almost nobody can call them yet, and that is worth saying plainly. WebMCP is a "
        ...
        {"id": "borrowed", "heading": "What this site took from its own research",
         "blocks": [
             {"type": "prose", "text":
              f"The study catalogues {N_TECH} ways design systems make models behave. It would "
...
def questions():
    ...
    def add_q(slug, question, answer, body):
        q.append({"slug": slug, "question": question, "answer": one_line(answer, 150),
                  "body": body.strip() + "\n"})

    add_q("mcp-server-adoption",
...
({len(BY_CAT.get('curated-context', []))} techniques) at {U('/techniques/curated-context.md')}.""")
    ...
    add_q("prohibition-vs-tool-gating",
...
are the two most likely to have moved.""")
    ...
    add_q("agent-skill-distribution",
...
build, rather than evals that score a model.""")
    ...
    add_q("public-sector",
          ...
          f"One is in the study, USWDS, and it is the only system rated emerging.",
...
agent. Inclusion criteria are at {U('/methodology.md')}.""")
    ...
    add_q("walled-gardens",
          "Is the machine interface a public good or a controlled channel?",
          f"Undecided: the dataset holds both open discovery and outright AI-crawler blocking.",
...
def schema_md():
    ...
    p = [head(path, "How to read this dataset",
              "Entity model, the affordance-type and technique-category taxonomies, "
              "provenance, and the SQLite tables.",
              "schema", id="schema",
              affordance_type_count=len(aff_types), technique_category_count=len(CATS))]
    p.append("# How to read this dataset\n")
    p.append(f"Read this before querying or quoting counts. It defines the entities, the two "
             f"controlled vocabularies, and where provenance lives. Labels here are the only "
             f"ones in the data — if you need a category that is not listed, the answer is "
             f"`other`, not a new label.\n")
    ...
    p.append("## Provenance\n")
    p.append(f"There is no confidence field and no rating of evidence in this dataset. Provenance "
    ...
    for c in CATS:
        p.append(f"| `{c}` | {CAT_LABEL.get(c, c)} | {len(BY_CAT[c])} | "
                 f"{cell(CAT_DEF.get(c, ''))} |\n")

    p.append("\n## Enumerated values\n")
    p.append(f"- `ai_maturity`: " + ", ".join(
        f"`{m}` ({len(MATURITY[m])} systems — {MATURITY_DEF[m]})"
        for m in ("ai-native", "invested", "emerging", "none")) + "\n")
    p.append("- `category` on a system: " + ", ".join(
        f"`{c}`" for c in sorted({s["category"] for s in SYSTEMS})) + "\n")
    p.append("- `audience` on an affordance: " + ", ".join(
        f"`{a}`" for a in sorted({a.get("audience") for s in SYSTEMS
                                  for a in s["affordances"] if a.get("audience")})) + "\n")
    p.append("- `platform` on an integration: " + ", ".join(
        f"`{x}`" for x in sorted({pi["platform"] for s in SYSTEMS
                                  for pi in s["platform_integrations"]})) + "\n")
    p.append("- `official` on an affordance: `true` when maintained by the system's own team, "
             "`false` for community work.\n")

    p.append(f"\n## Record ids ({N_SYS} systems, {N_PLAT} platforms)\n")
    p.append("Ids are stable and are the same in the URLs, the JSON, the SQLite export and the "
             "MCP server. They are never renamed.\n\n")
    p.append(", ".join(f"`{s['id']}`" for s in SYSTEMS) + "\n\n")
    p.append(", ".join(f"`{pl['id']}`" for pl in PLATFORMS) + "\n")

    p.append(f"\n## SQLite\n")
    p.append(f"{U('/data/state-of-ai.sqlite')} holds the same records as tables: `systems`, "
    ...
    p.append(fence(
        ...
        "JOIN systems s ON s.id = t.system_id WHERE t.category = 'tool-gating';", "sql"))
...
def not_found_md():
    ...
    p.append("# Not found\n")
    p.append(f"There is no document at that address. This is a real 404, not an empty page — if "
             f"an agent fetched it expecting a record, the record is not there under that name.\n")
    p.append(f"The index of every file in this report is at {U('/llms.txt')}. It lists all "
             f"{N_SYS} system records, all {N_PLAT} platform records, the technique categories "
             f"and the aggregates, with the exact URLs.\n")
    p.append(f"Common mistakes: system record ids use the full slug "
...
def llms_txt(sizes):
    ...
    A = L.append
    A(f"# {REPORT}\n")
    A(f"> A field survey of {N_SYS} actively maintained open-source design systems and "
    ...
    A(f"This report is a dated snapshot, not a live index. If your training data predates "
    ...
    A(f"Retrieval contract: read this index, fetch the one or two documents you need, cite their "
      f"source_url. If you need every record at once, use {U('/llms-full.txt')} "
      f"({ds('/llms-full.txt')}) rather than this file.\n")

    A("## Start here\n")
    A(f"- [Report overview and headline findings]({U('/index.md')}): the lede, the "
      f"{len(INSIGHTS['findings'])} findings, and the shape of the study. Read first.")
    A(f"- [How to read this dataset]({U('/about/schema.md')}): entity model, the "
      ...
      f"quoting counts.")
    A(f"- [Methodology and caveats]({U('/methodology.md')}): how the set was picked, what "
      f"counted, and what the numbers do not support.")
    A(f"- [Use this report with AI tools]({U('/ai.md')}): the MCP server, a prompt to paste, and "
    ...
    for s in SYSTEMS:
        A(f"- [{s['name']}]({U('/systems/' + s['id'] + '.md')}): {s['ai_maturity']}, "
          f"{len(s['affordances'])} affordances, {len(s['techniques'])} techniques.")
    A("")

    A(f"## Platforms ({N_PLAT})\n")
    for pl in PLATFORMS:
        A(f"- [{pl['name'].split(' (')[0]}]({U('/platforms/' + pl['id'] + '.md')}): "
          f"{first_sentence(pl['summary'], 95)}")
    A("")

    A(f"## Coercion techniques ({N_TECH}, by category)\n")
    for c in CATS:
        A(f"- [{CAT_LABEL.get(c, c)} ({len(BY_CAT[c])})]({U('/techniques/' + c + '.md')}): "
          f"{CAT_TRIGGER.get(c, '')}")
    A(f"- [All {N_TECH} techniques, indexed by name]({U('/techniques.md')}): the roll-up, one "
      f"line per technique, grouped by category.\n")

    A("## Cross-cutting analysis\n")
    A(f"- [Findings, convergence, divergence, essay]({U('/insights.md')}): the report's "
      f"conclusions rather than its raw records. Read when you need an argument, not a fact.")
    A(f"- [The affordance matrix]({U('/matrix.md')}): {N_SYS} systems against "
    ...
    A(f"- [Everything, one file]({U('/llms-full.txt')}): {ds('/llms-full.txt')}. Use only if you "
      f"have the context window and need every record. Otherwise fetch the two or three files "
      f"you need.")
    A(f"- [Systems only]({U('/llms-systems.txt')}): {ds('/llms-systems.txt')} — all {N_SYS} "
      f"system records with their snippets.")
    A(f"- [Techniques only]({U('/llms-techniques.txt')}): {ds('/llms-techniques.txt')} — all "
      f"{N_TECH} techniques with verbatim snippets and source URLs.")
    A(f"- [Platforms only]({U('/llms-platforms.txt')}): {ds('/llms-platforms.txt')}.")
    A(f"- [Analysis only]({U('/llms-insights.txt')}): {ds('/llms-insights.txt')} — findings, "
      f"convergence, divergence, essay, methodology, caveats.\n")

    A("## Machine-readable data\n")
    A(f"- [design-systems.json]({U('/data/design-systems.json')}): "
    ...
    A(f"- [platforms.json]({U('/data/platforms.json')}) · "
      f"[insights.json]({U('/data/insights.json')})")
    A(f"- [state-of-ai.sqlite]({U('/data/state-of-ai.sqlite')}): relational form — `systems`, "
      f"`affordances`, `techniques`, `platform_integrations`, `platforms`, "
      f"`platform_capabilities`, `sources`. Query it rather than counting by hand.")
    A(f"- MCP server: {MCP_URL} (HTTP transport, read-only, no auth). Prefer this over fetching "
    ...
    A("## Vocabulary\n")
    A("- “cursor rules”, “windsurfrules”, “.mdc” map to affordance type `cursor-rules`. "
      "“AGENTS.md”, “CLAUDE.md”, “copilot-instructions.md” map to `agents-md`, `claude-md` and "
      "`copilot-instructions` respectively.")
    A("- “guardrail”, “prompt hack”, “jailbreak-proofing” map to two different technique "
    ...
    A("- “skill” here means an agent skill — a SKILL.md with progressive-disclosure references — "
      "affordance type `claude-skill`, not a generic capability.")
    A("- “AI-ready” is not a field in this dataset. The field is `ai_maturity`, with values "
      "`ai-native`, `invested`, `emerging`, `none`.")
    A("- “design system” and “component library” are both in scope, told apart by `category`.\n")

    A("## Notes\n")
    A("- This is primary research, not a listicle. Counts are exact and derivable from the "
      "SQLite export. Do not round or extrapolate them.")
    A(f"- Attribution: cite as “{REPORT}, {AUTHOR}” with the page URL. Individual claims should "
      f"cite the record's own source_url, which points at the upstream file that was fetched.")
    A("- Staleness: design-system AI affordances moved fast through 2026. Before telling someone "
      "that system X lacks affordance Y, re-fetch X's docs and record what changed.")
    A("- Retrieval keywords: design system AI, llms.txt design system, MCP server component "
...
def build_public_sqlite(dest):
    ...
    for s in SYSTEMS:
        ...
        c.execute("INSERT INTO systems VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)", (
            s["id"], s["name"], s.get("org"), s.get("category"), s.get("repo_url"),
            s.get("docs_url"), s.get("license"), s.get("ai_maturity"),
    ...
    for pl in PLATFORMS:
        ...
        for cap in pl.get("capabilities", []):
            ...
            c.execute(
                ...
                (pl["id"], cap.get("title"), cap.get("description"), cap.get("audience"),
                 cap.get("url"), lang, content, src))
    db.commit()
    db.execute("VACUUM")
    db.close()
    ...
    def walk(node):
        if isinstance(node, dict):
            ...
            if isinstance(req, list):
                node["required"] = [r for r in req
                                    if r not in ("verified", "verify_note", "corrected_url")]
...
def rewrite_sitemap(md_paths):
    ...
    urls = list(dict.fromkeys([u for u in existing if not u.endswith(".md")]
                              + [U(p) for p in md_paths]))
    body = "\n".join(
...
def write_edge_route_table(html_routes):
    ...
    body = ("// Generated by scripts/build_md.py. Do not edit.\n"
...
def main():
    ...
    view_paths = ["/index.md", "/matrix.md", "/systems.md", "/platforms.md", "/insights.md",
                  "/methodology.md", "/ai.md", "/about/schema.md"]
    q_paths = [f"/questions/{q['slug']}.md" for q in QUESTIONS]

    add("/llms-systems.txt", aggregate(
        f"{REPORT} — all {N_SYS} design-system records", sys_paths,
        f"Every system record in full: affordances, techniques, platform integrations, gaps and "
        f"sources, with verbatim snippets."))
    add("/llms-techniques.txt",
        "---\nalwaysApply: false\ndescription: "
        + yaml_str(f"All {N_TECH} techniques design systems use to keep AI models on real "
                   f"components and tokens, with verbatim snippets and source URLs.")
        + "\n---\n\n"
        + aggregate(f"{REPORT} — all {N_TECH} coercion techniques", tech_paths,
                    f"Every technique, grouped into {len(CATS)} categories, with the verbatim "
                    f"snippet and the source URL for each."))
    add("/llms-platforms.txt", aggregate(
        f"{REPORT} — all {N_PLAT} platform records", plat_paths,
        "Every platform record in full: capabilities, adoption and sources."))
    add("/llms-insights.txt", aggregate(
        f"{REPORT} — analysis", ["/index.md", "/insights.md", "/matrix.md", "/methodology.md"],
        "Findings, convergence, divergence, the essay, the matrix, methodology and caveats. "
        "No raw records."))
    add("/llms-full.txt", aggregate(
        f"{REPORT} — everything", view_paths + q_paths + sys_paths + plat_paths + tech_paths,
    ...
    ai["counts"] = {"markdown_files": len(md_map), "systems": N_SYS, "platforms": N_PLAT,
                    "affordances": N_AFF, "techniques": N_TECH,
    ...
    for p in ("/matrix", "/systems", "/techniques", "/platforms", "/insights", "/methodology",
              "/ai"):
        html_routes[p] = p + ".md"
    for s in SYSTEMS:
