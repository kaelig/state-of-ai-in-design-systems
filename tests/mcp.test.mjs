...
async function legacy(method, params = {}) {
    const response = await post({ jsonrpc: '2.0', id: nextId++, method, params }, { 'mcp-protocol-version': '2025-06-18' });
...
async function modern(method, params = {}) {
    ...
    const response = await post({ jsonrpc: '2.0', id: nextId++, method, params: { ...params, _meta: ENVELOPE } }, headers);
    const raw = await response.text();
    return { response, raw, body: JSON.parse(raw) };
}
...
async function callTool(name, args = {}) {
    ...
    assert.ok(body.result, `tools/call ${name} returned no result: ${JSON.stringify(body).slice(0, 400)}`);
    return body.result;
}

const firstText = result => result.content.map(c => c.text ?? '').join('');
const callJson = async (name, args) => JSON.parse(firstText(await callTool(name, args)));
...
describe('legacy 2025 leg', () => {
    ...
    test('resources/list and resources/read', async () => {
        ...
        assert.ok(uris.includes('dsai://system/ant-design'), `missing system resource in ${uris.slice(0, 5)}`);
        assert.ok(uris.includes('dsai://report/methodology'));

        const { body: read } = await legacy('resources/read', { uri: 'dsai://system/ant-design' });
        assert.equal(read.result.contents[0].mimeType, 'text/markdown');
        assert.equal(read.result.contents[0].text, MD_MAP['/systems/ant-design.md']);
    });
...
describe('modern 2026-07-28 leg', () => {
    ...
    test('tools/list and tools/call round-trip', async () => {
        ...
        const { body: called } = await modern('tools/call', { name: 'get_stats', arguments: {} });
    ...
    test('a second modern call through the same module-scope handler works', async () => {
        const { body } = await modern('tools/call', { name: 'get_platform', arguments: { id: 'figma' } });
        const platform = JSON.parse(body.result.content.map(c => c.text).join(''));
...
describe('tool surface', () => {
    ...
    test('every tool declares itself read-only and describes itself', async () => {
        ...
        for (const tool of body.result.tools) {
            assert.equal(tool.annotations?.readOnlyHint, true, `${tool.name} is not marked readOnlyHint`);
            assert.ok(tool.description.length > 120, `${tool.name} description is too thin`);
            assert.match(tool.description, /2026-07-27/, `${tool.name} does not mention the snapshot date`);
        }
    });
});
...
describe('context budget', () => {
    ...
    test('the default get_system for every system stays under 32KB', async () => {
        for (const system of PAYLOAD.systems) {
            const size = firstText(await callTool('get_system', { id: system.id })).length;
            assert.ok(size < 32 * 1024, `get_system ${system.id} returned ${size} bytes`);
        }
    });

    test('get_system strips snippet bodies unless asked', async () => {
        const leanText = firstText(await callTool('get_system', { id: 'ant-design' }));
        const fatText = firstText(await callTool('get_system', { id: 'ant-design', include: ['snippets'] }));
        assert.ok(!leanText.includes('"snippet"'), 'snippet bodies leaked into the default get_system');
        assert.ok(leanText.includes('"snippet_ref"'), 'no snippet_ref handed back');
        assert.ok(fatText.includes('"content"'), 'include:["snippets"] did not add the bodies');
        assert.ok(fatText.length > leanText.length * 1.4, `snippets added only ${fatText.length - leanText.length} bytes`);
    });
    ...
    test('pagination reports a next_offset when there is more', async () => {
        ...
        const second = await callJson('list_affordances', { limit: 10, offset: 10 });
...
describe('markdown is byte-identical to the static mirrors', () => {
    test('get_system format markdown', async () => {
        for (const id of ['shadcn-ui', 'uswds', 'ant-design']) {
            const md = firstText(await callTool('get_system', { id, format: 'markdown' }));
    ...
    test('get_report with no section lists the sections', async () => {
        ...
        assert.equal(toc.sections.length, Object.keys(MD_MAP).filter(isReportPath).length);
    });
});
...
function isReportPath(path) {
    return (
        ...
        ['/index.md', '/systems.md', '/matrix.md', '/techniques.md', '/platforms.md', '/insights.md', '/methodology.md', '/ai.md', '/about/schema.md'].includes(
            path
        )
    );
}
...
describe('get_stats matches the payload', () => {
    test('counts are strictly equal to counts derived from payload.json', async () => {
        ...
        assert.equal(
            ...
            [...affordances, ...techniques, ...capabilities].filter(x => x.snippet).length
        );
    ...
    test('enums cover every value present in the data', async () => {
        ...
        for (const type of Object.keys(stats.affordance_types)) {
            assert.ok(stats.enums.affordance_type.includes(type), `${type} missing from the enum`);
        }
        assert.deepEqual(stats.enums.system_id, PAYLOAD.systems.map(s => s.id));
        assert.deepEqual(stats.enums.platform, PAYLOAD.platforms.map(p => p.id));
    });
});

describe('no research-process fields leak', () => {
    const FORBIDDEN = ['verify_note', '"verified"', 'verify_status', '"corrected"'];

    test('get_system, list_techniques, get_snippet and search bodies are clean', async () => {
        const bodies = [
            firstText(await callTool('get_system', { id: 'ant-design', include: ['snippets'] })),
            firstText(await callTool('get_system', { id: 'carbon-design-system', include: ['snippets'] })),
            firstText(await callTool('list_techniques', { limit: 100 })),
            firstText(await callTool('list_affordances', { limit: 100 })),
            firstText(await callTool('get_snippet', { ref: 'technique:ant-design:0' })),
...
describe('search', () => {
    test('finds techniques and hands back a usable snippet_ref', async () => {
        const result = await callJson('search', { query: 'copilot instructions', kind: 'technique', limit: 5 });
    ...
    test('system_id narrows the results', async () => {
        const result = await callJson('search', { query: 'mcp', system_id: 'shadcn-ui', limit: 50 });
...
describe('errors are errors, not crashes', () => {
    test('unknown tool comes back as a JSON-RPC error', async () => {
        const { response, body } = await legacy('tools/call', { name: 'no_such_tool', arguments: {} });
        assert.equal(response.status, 200);
        assert.ok(body.error || body.result?.isError, 'expected an error, got a plain result');
    });

    test('get_system with a bad id lists the valid ids', async () => {
        const { body } = await legacy('tools/call', { name: 'get_system', arguments: { id: 'not-a-system' } });
        const message = body.result.content.map(c => c.text).join('');
        assert.equal(body.result.isError, true);
        assert.match(message, /not-a-system/);
        for (const id of ['shadcn-ui', 'uswds']) assert.ok(message.includes(id), `${id} not listed`);
    });

    test('get_report with a bad section lists the valid sections', async () => {
        const { body } = await legacy('tools/call', { name: 'get_report', arguments: { section: 'nope' } });
        const message = body.result.content.map(c => c.text).join('');
        assert.equal(body.result.isError, true);
    ...
    test('get_snippet with a bad ref explains the ref format', async () => {
        const { body } = await legacy('tools/call', { name: 'get_snippet', arguments: { ref: 'technique:nope:99' } });
        const message = body.result.content.map(c => c.text).join('');
        assert.equal(body.result.isError, true);
    ...
    test('a bad enum value is rejected by schema validation', async () => {
        const { body } = await legacy('tools/call', { name: 'list_systems', arguments: { maturity: 'wishful' } });
...
describe('HTTP negative cases', () => {
    ...
    test('legacy POST accepting only application/json is 406', async () => {
        const response = await post({ jsonrpc: '2.0', id: 900, method: 'tools/list', params: {} }, { accept: 'application/json' });
